...
FILE_NYAN_BOT=.nyanbot.beetnodes
...
function check_instalacion { ### Check instalacion
    if [ -f "$FILE_CUSTOM" ]; then
    ...
    elif [ -f "$FILE_NYAN_BOT" ]; then
        echo -e '\e[92m* \e[39m[\e[94mINFO\e[39m] Acabas de iniciar tu servidor, y has seleccionado \e[4m\e[95mNyanBot\e[39m\e[0m!'
            sleep 3
                    echo
            sleep 2
                clear
        echo -e '\e[92m* \e[39m[\e[94mINFO\e[39m] Iniciando servidor...'
            sleep 2.5
                echo
                    /usr/local/bin/npm install node-tesseract-ocr;
                    /usr/local/bin/npm install @adiwajshing/baileys;
                    /usr/local/bin/npm install ffmpeg;
